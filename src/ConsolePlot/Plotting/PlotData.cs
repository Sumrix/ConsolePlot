...
internal class PlotData
{
    ...
    public static PlotData Calculate(List<Series> series, PlotSettings settings, int width, int height)
    {
        ...
        {
            return new(initialBounds, new(0, 0, width, height), [], [], new(0, 0), series);
        }
        ...
        return new(adjustedBounds, drawingArea, xTicks, yTicks, axisCross, series);
    }
    ...
    {
        var xValues = series.SelectMany(s => s.Xs).Where(double.IsFinite);
        var yValues = series.SelectMany(s => s.Ys).Where(double.IsFinite);

        return new(xValues.Min(), xValues.Max(), yValues.Min(), yValues.Max());
    }
        ...
        int axisLabelWidth)
    {
        ...
        var cross = ticks.MinBy(t => Math.Abs(t.Value))!.Value;
        ...
        if (settings.Ticks.Labels is { IsVisible: true, AttachToAxis: true })
        {
    ...
    {
        return new(CalculateAxisCross(xTicks), CalculateAxisCross(yTicks));
    }

    private static double CalculateAxisCross(List<Tick> ticks)
    {
        return ticks.OrderBy(t => Math.Abs(t.Value)).First().Value;
    }
        ...
        int height)
    {
        ...
        return new(width - drawingAreaX, height - drawingAreaY, drawingAreaX, drawingAreaY);
    }

    private static int CalculateDrawingRange(PlotSettings settings, int labelSize, int size)
    {
        return settings.Ticks.Labels is { IsVisible: true, AttachToAxis: false }
            ? size - labelSize
            : size;
    }
}